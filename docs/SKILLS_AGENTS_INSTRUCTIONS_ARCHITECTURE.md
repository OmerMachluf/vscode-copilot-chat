...
## Claude Code Sync Mechanism

The extension auto-generates Claude Code configuration from the source files:

### Generated Files

| Source | Generated | Purpose |
|--------|-----------|---------|
| `assets/agents/*.agent.md` | `.claude/agents/builtin/*.md` | Claude agent definitions |
| `.github/agents/*.agent.md` | `.claude/agents/custom/*.md` | Custom agent definitions |
| All agents | `.claude/commands/*.md` | Slash commands (e.g., `/architect`) |
| `.github/skills/` | `.claude/skills/{id}/SKILL.md` | Skill files |
| Composite | `CLAUDE.md` | Project instructions |
| Generated | `.claude/settings.json` | Configuration |
...
Extension Activation
         ↓
runClaudeMigration() checks:
  - CLAUDE.md exists and is current version?
  - All command files exist?
  - Source files haven't changed?
         ↓
If any check fails → ClaudeMigrationService.migrate()
         ↓
File Watcher monitors: **/.github/{agents,instructions}/**/*.md
  - On change → auto-regenerate
```

**Source**: `src/extension/extension/vscode-node/extension.ts:68-106`
...
This allows the system to detect outdated files and regenerate when the format changes.
...
| `ClaudeMigrationService` | `claudeMigrationService.ts` | Syncs to Claude Code format |
...
2. The sync mechanism will auto-generate:
